      ...
      <div class="container d-flex justify-content-between align-items-center relative-container">
                    ...
                    <div class="mega-content-wrapper">
                        ...
                        <div class="mega-column brands-column" v-if="getMegaDataForCategory(category.id).brands.length > 0">
                            ...
                            <div class="brands-list-vertical">
                                <a href="#" 
                                   v-for="brand in getMegaDataForCategory(category.id).brands" 
                                   :key="brand" 
                                   class="brand-link"
                                   @click.prevent="goToShopWithBrand(category.id, brand)">
                                    {{ brand }}
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="mega-column products-column border-end">
                            ...
                            <ul class="mega-product-list">
                                <li v-for="prod in getMegaDataForCategory(category.id).products" :key="prod.id" @click.stop="goToProduct(prod.id)">
                                    <div class="search-item">
                                        <!-- Ảnh to hơn -->
                                        <img :src="getFullImageUrl(prod.thumbnail_url || prod.image_url)" class="search-item-img big-img" alt="img"
                        ...
                        <div class="mega-column products-column">
                            ...
                            <ul class="mega-product-list">
                                <li v-for="prod in getMegaDataForCategory(category.id).topRated" :key="prod.id" @click.stop="goToProduct(prod.id)">
                                    <div class="search-item">
                                        <!-- Ảnh to hơn -->
                                        <img :src="getFullImageUrl(prod.thumbnail_url || prod.image_url)" class="search-item-img big-img" alt="img"
                                            @error="$event.target.src='https://placehold.co/50x50?text=No+Img'">
        ...
        <div class="search-bar-wrapper" ref="searchContainer">
            ...
            <div class="search-results-dropdown" v-if="showSearchResults && searchQuery.length > 0">
                ...
                <div v-else-if="!isFetchingSearch && categoryResults.length === 0 && searchResults.length === 0 && searchQuery.length > 1" class="p-3 text-center text-muted">
        ...
        <div class="header-actions">
          ...
          <div v-else class="side-menu-container user-menu" ref="userMenuContainer">
            ...
            <div class="dropdown-menu user-dropdown" :class="{ active: isUserMenuActive }">
              ...
              <ul class="menu-list">
                <li><router-link :to="{ name: 'profile' }" class="lordicon-link profile-target"><div class="icon-wrap"><lord-icon src="https://cdn.lordicon.com/bhfjfgqz.json" trigger="hover" target=".profile-target" colors="primary:#4b5563" style="width:20px;height:20px"></lord-icon></div>Tài khoản</router-link></li>
                <li><router-link :to="{ name: 'OrderList' }" class="lordicon-link order-target"><div class="icon-wrap"><lord-icon src="https://cdn.lordicon.com/pbrgppbb.json" trigger="hover" target=".order-target" colors="primary:#4b5563" style="width:20px;height:20px"></lord-icon></div>Đơn mua</router-link></li>
                <li><router-link :to="{ name: 'wishlist' }" class="lordicon-link wishlist-target"><div class="icon-wrap"><lord-icon src="https://cdn.lordicon.com/rjzlnunf.json" trigger="hover" target=".wishlist-target" colors="primary:#4b5563,secondary:#e91e63" style="width:20px;height:20px"></lord-icon></div>Yêu thích</router-link></li>
                <li class="divider"><hr></li>
                <li><a href="#" class="logout-link lordicon-link logout-target" @click.prevent="handleLogout"><div class="icon-wrap"><lord-icon src="https://cdn.lordicon.com/moscwhoj.json" trigger="hover" target=".logout-target" colors="primary:#d9534f,secondary:#d9534f" style="width:20px;height:20px"></lord-icon></div>Đăng xuất</a></li>
              </ul>
            </div>
          </div>
        </div>
...
<style>
.elegant-popup { border-radius: 16px !important; font-family: Arial, sans-serif !important; box-shadow: 0 20px 60px rgba(0,0,0,0.15) !important; padding: 1.5rem !important; }
.elegant-title { font-size: 1.5rem !important; color: #333 !important; font-weight: 700 !important; }
.elegant-confirm-btn { border-radius: 8px !important; padding: 12px 24px !important; font-weight: 600 !important; background-color: #009981 !important; border: none !important; transition: transform 0.2s !important; }
.elegant-confirm-btn:hover { transform: translateY(-2px); background-color: #00806b !important; }
.elegant-cancel-btn { border-radius: 8px !important; padding: 12px 24px !important; font-weight: 600 !important; background-color: #f3f4f6 !important; color: #4b5563 !important; border: 1px solid #e5e7eb !important; transition: background-color 0.2s !important; }
.elegant-cancel-btn:hover { background-color: #e5e7eb !important; color: #374151 !important; }
mark { background-color: transparent; color: #009981; padding: 0; font-weight: 800; text-decoration: underline; }
</style>
...
/* CATEGORY BUTTON */
.category-button { background-color: var(--bg-gray); border: 1px solid #eee; color: var(--text-dark); padding: 10px 15px; border-radius: 8px; cursor: pointer; display: flex; align-items: center; gap: 8px; font-size: 14px; white-space: nowrap; transition: all 0.2s; }
...
.dropdown-menu { 
  ...
  display: none; z-index: 1001; border: 1px solid #eee; 
  padding: 5px 0; 
...
.menu-list { position: relative; } /* Giúp định vị các item */
...
.menu-list li > a { 
    ...
    padding: 12px 15px; color: var(--text-dark); font-size: 14px; 
...
.mega-menu-panel {
    position: absolute;
    left: 100%; 
    top: 0; /* Luôn bắt đầu từ đỉnh menu cha */
    
    width: 1070px; /* [UPDATED] Tăng độ rộng mega menu */
    min-height: 100%; 
...
.brand-link:hover {
    ...
    background: #f0fdfa;
    padding-left: 14px; /* Hiệu ứng đẩy nhẹ */
}

/* Cột Sản phẩm */
.products-column {
    width: 40%; /* Chiếm 40% mỗi cột */
    background: #fff;
}
...
.arrow-icon { font-size: 10px; color: #ccc; }

/* SEARCH BAR */
.search-bar-wrapper { flex-grow: 1; position: relative; margin-left: 20px; }
.search-bar { width: 100%; position: relative; }
.search-bar input { width: 100%; padding: 10px 15px; border: 2px solid var(--primary-color); border-radius: 8px; font-size: 14px; outline: none; background-color: var(--bg-gray); }
.search-bar input:focus { background-color: #fff; box-shadow: 0 0 0 3px rgba(0, 153, 129, 0.1); }
.search-btn { position: absolute; right: 5px; top: 50%; transform: translateY(-50%); border: none; background: var(--primary-color); color: white; padding: 6px 15px; border-radius: 6px; cursor: pointer; height: 80%; transition: 0.2s; }
.search-btn:hover { background-color: var(--primary-dark); }

/* SEARCH RESULTS DROPDOWN (INSTANT) */
.search-results-dropdown { position: absolute; top: 100%; left: 0; right: 0; background: #fff; border: 1px solid #eee; border-radius: 8px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); z-index: 1002; margin-top: 8px; overflow: hidden; }
.search-header-label { padding: 10px 15px; font-size: 12px; color: #666; background: #f8f9fa; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1px solid #eee; border-top: 1px solid #eee; }
.search-header-label:first-child { border-top: none; }
.search-list { list-style: none; padding: 0; margin: 0; max-height: 400px; overflow-y: auto; }
.search-list::-webkit-scrollbar { width: 6px; }
.search-list::-webkit-scrollbar-thumb { background-color: #ccc; border-radius: 3px; }
.search-list li { cursor: pointer; border-bottom: 1px solid #f1f1f1; transition: background 0.2s; }
.search-list li:last-child { border-bottom: none; }
.search-list li:hover { background-color: #f0fdfa; }
.search-item { display: flex; align-items: center; padding: 10px; }
.search-item-img { width: 40px; height: 40px; object-fit: cover; border-radius: 4px; margin-right: 12px; border: 1px solid #eee; background: #fff; }
/* [UPDATED] Style cho ảnh to hơn trong mega menu */
.search-item-img.big-img { width: 60px; height: 60px; margin-right: 15px; }

.search-item-info { flex: 1; min-width: 0; }
.search-item-name { font-size: 13px; font-weight: 500; color: #333; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.3; }
.search-item-price { font-size: 13px; color: #d9534f; font-weight: 700; margin-top: 2px; }
.category-item { padding: 10px 15px; }
.cat-icon { width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; background: #e6fffa; color: #009981; border-radius: 50%; margin-right: 10px; font-size: 14px; }
.cat-name { font-weight: 600; color: #333; }
.search-footer { background-color: #fcfcfc; }
.search-footer a:hover { text-decoration: underline !important; color: var(--primary-dark) !important; }

/* HEADER ACTIONS */
.header-actions { display: flex; align-items: center; gap: 15px; margin-left: 20px; }
.action-item { display: flex; align-items: center; gap: 8px; padding: 8px 12px; border-radius: 8px; background-color: var(--bg-gray); color: var(--text-dark); font-size: 14px; white-space: nowrap; cursor: pointer; transition: all 0.2s; }
...
.cart-icon-wrapper { position: relative; display: flex; align-items: center; margin-right: 5px; }
.cart-badge { position: absolute; top: -8px; right: -8px; background-color: var(--danger-color); color: white; font-size: 10px; font-weight: bold; min-width: 16px; height: 16px; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: 2px solid #fff; padding: 0 2px; z-index: 10; }
...
.user-avatar-mini { width: 32px; height: 32px; border-radius: 50%; object-fit: cover; border: 1px solid #eee; }
.user-avatar-placeholder { width: 32px; height: 32px; border-radius: 50%; background-color: var(--primary-light); color: var(--primary-dark); display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 14px; border: 1px solid var(--primary-color); }
...
.icon-wrap { display: flex; align-items: center; justify-content: center; margin-right: 8px; width: 24px; }
