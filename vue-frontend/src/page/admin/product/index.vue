...
const requireLogin = () => {
    ...
    if (!hasRole(['admin', 'staff'])) {
        Swal.fire({ icon: 'warning', title: 'Quyền hạn', text: 'Bạn không có quyền quản lý sản phẩm.' });
        return false;
    }
    return true;
};
...
const viewingProduct = ref({
    name: '', description: '', categoryName: '', status: '', thumbnail_url: '',
    images: [], variants: [], attributeNames: [], priceRange: '', totalStock: 0
});
...
const errors = reactive({ name: '', category_id: '', variants: '', attributes: '' });
...
function flattenVariantAttributes(variant) {
    ...
    if (Array.isArray(variant.attribute_values)) {
        variant.attribute_values.forEach(av => {
            if (av.attribute && av.attribute.name) attrs[av.attribute.name] = av.value;
        });
...
const searchedAndSortedProducts = computed(() => {
    ...
    if (query) {
        result = result.filter(p => p.name.toLowerCase().includes(query) || (p.category?.name && p.category.name.toLowerCase().includes(query)));
    }
...
const isAllImagesSelected = computed({
    get() { return formData.existing_images.length > 0 && selectedImageIds.value.length === formData.existing_images.length; },
    set(val) { selectedImageIds.value = val ? formData.existing_images.map(img => img.id) : []; }
});

// Watchers
watch([searchQuery, selectedCategory, sortCriteria, activeTab], () => { currentPage.value = 1; });

// ==========================================
// HELPERS
// ==========================================
const formatCurrency = (value) => value == null || isNaN(value) ? 'N/A' : new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(value);
const getMinPrice = (variants) => {
    if (!variants?.length) return 0;
    const prices = variants.map(v => parseFloat(v.price)).filter(p => !isNaN(p));
    return prices.length ? Math.min(...prices) : 0;
};
const calculateTotalStock = (variants) => variants?.reduce((acc, v) => acc + (parseInt(v.stock) || 0), 0) || 0;
...
async function handleCreateQuickAttribute() {
    if (!newQuickAttrName.value.trim()) return Swal.fire('Lỗi', 'Vui lòng nhập tên thuộc tính', 'warning');
    ...
    } catch (error) { Swal.fire('Lỗi', error.response?.data?.message || 'Không thể tạo thuộc tính.', 'error'); } 
    finally { isCreatingAttr.value = false; }
}
...
function addSelectedAttributeToForm() {
    ...
    formData.variants.forEach(v => { if (!v.attributes) v.attributes = {}; v.attributes[attrObj.name] = ''; });
...
function addVariantRow() {
    ...
    formData.variants.push(reactive({ variant_id: generateShortId(), original_price: 0, price: 0, stock: 0, attributes: newAttributes }));
}
...
function openCreateModal() { 
    ...
    resetForm(); isEditMode.value = false; modalInstance.value?.show(); 
}
...
async function openVariantConfigModal(productItem) {
    if (!requireLogin()) return;
    resetForm(); isEditMode.value = true; modalInstance.value?.show();
    try {
        ...
        if (variants.length > 0) {
            ...
            variants.forEach(v => { if (v.attributes) Object.keys(v.attributes).forEach(key => allAttributeNames.add(key)); });
            formData.attribute_definitions = reactive(Array.from(allAttributeNames).map(name => {
            ...
            formData.variants = reactive(variants.map(v => {
                ...
                formData.attribute_definitions.forEach(def => { variantAttributes[def.name] = ''; });
                if (v.attributes) Object.entries(v.attributes).forEach(([key, value]) => { variantAttributes[key] = value; });
                return { variant_id: v.id, original_price: v.original_price || 0, price: v.price || 0, stock: v.stock || 0, attributes: variantAttributes };
            }));
        } else { addVariantRow(); }
    } catch (e) { Swal.fire('Lỗi', 'Không thể tải chi tiết sản phẩm.', 'error'); }
}
...
async function openViewModal(productItem) {
    try {
        const res = await apiService.get(`/admin/products/${productItem.id}`);
        const product = res.data.data || res.data;
        const processedVariants = (product.variants || []).map(v => ({ ...v, attributes: flattenVariantAttributes(v) }));
        const keys = new Set(); processedVariants.forEach(v => Object.keys(v.attributes).forEach(k => keys.add(k)));
        
        viewingProduct.value = {
            ...product, categoryName: product.category?.name || 'N/A', attributeNames: Array.from(keys),
            images: (product.images || []).map(img => getImageUrl(img.image_url)),
            thumbnail_url: getImageUrl(product.thumbnail_url), priceRange: getPriceRange(product.variants),
            totalStock: calculateTotalStock(product.variants), variants: processedVariants
        };
        viewModalInstance.value?.show();
    } catch (e) { Swal.fire('Lỗi', 'Không thể xem chi tiết.', 'error'); }
}
...
async function deleteExistingImage(imgId, index) {
    ...
    if (!(await Swal.fire({ title: 'Xóa ảnh?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33' })).isConfirmed) return;
    ...
    try {
        ...
        Swal.fire({ icon: 'success', title: 'Đã xóa ảnh', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
    } catch (e) { Swal.fire('Lỗi', 'Không thể xóa ảnh', 'error'); } 
    finally { deletingImageIds.value = deletingImageIds.value.filter(id => id !== imgId); }
}

async function handleBulkDeleteImages() {
    if (!requireLogin()) return;
    if (selectedImageIds.value.length === 0) return;
    if (!(await Swal.fire({ title: 'Xóa nhiều ảnh?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33' })).isConfirmed) return;
    ...
    if (isEditMode.value && formData.variants.length > 0) {
        for (const v of formData.variants) {
            ...
            if (isNaN(price) || price < 0) {
                 errors.variants = 'Giá bán không được âm.'; isValid = false; break;
            }
            if (price > 0 && price < MIN_PRICE) {
                 errors.variants = `Giá bán tối thiểu là ${formatCurrency(MIN_PRICE)}.`; isValid = false; break;
            }
            if (price > MAX_PRICE) {
                 errors.variants = `Giá bán quá lớn (> ${formatCurrency(MAX_PRICE)}). Vui lòng kiểm tra lại.`; isValid = false; break;
            }

            // Validate Stock
            const stock = parseInt(v.stock);
            if (isNaN(stock) || stock < 0) {
                 errors.variants = 'Số lượng kho không được âm.'; isValid = false; break;
            }

            // Validate Attributes
            for (const def of formData.attribute_definitions) {
                if (!v.attributes[def.name] || !v.attributes[def.name].toString().trim()) { 
                    errors.variants = `Vui lòng điền giá trị cho "${def.name}"`; isValid = false; break; 
                }
            }
            if (!isValid) break;
        }
...
async function handleSave() {
    if (!requireLogin()) return;
    if (!validateForm()) {
        Swal.fire('Thông tin chưa hợp lệ', 'Vui lòng kiểm tra lại các trường báo đỏ.', 'warning');
        return;
    }
    isSaving.value = true;
    try {
        ...
        if (!isEditMode.value) {
            ...
            await apiService.post(`/admin/products/${savedProductId}`, productData, { headers: { 'Content-Type': 'multipart/form-data' } });
        }

        if (savedProductId && galleryFiles.value.length > 0) {
            const uploadPromises = galleryFiles.value.map(file => {
                const imgData = new FormData(); imgData.append('product_id', savedProductId); imgData.append('image', file);
                return apiService.post('/admin/imageProducts', imgData, { headers: { 'Content-Type': 'multipart/form-data' } });
            });
        ...
        if (isEditMode.value) {
            for (const variant of formData.variants) {
                const variantPayload = { product_id: savedProductId, original_price: variant.original_price, price: variant.price, stock: variant.stock };
                ...
                } else { await apiService.patch(`/admin/variants/${savedVariantId}`, variantPayload); }
                
                if (savedVariantId && Object.keys(variant.attributes).length > 0) {
                    await apiService.post(`/admin/variants/${savedVariantId}/attributes`, { attributes: variant.attributes });
                }
            }
            Swal.fire('Thành công', 'Đã cập nhật!', 'success');
        }
        modalInstance.value?.hide(); fetchProducts();
    } catch (e) { Swal.fire('Lỗi', e.response?.data?.message || 'Lỗi khi lưu.', 'error'); } 
    finally { isSaving.value = false; }
}

async function handleDelete(product) {
    if (!requireLogin()) return;
    if ((await Swal.fire({ title: 'Xóa?', text: 'Không thể hoàn tác!', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33' })).isConfirmed) {
        try { await apiService.delete(`/admin/products/${product.id}`); fetchProducts(); Swal.fire('Đã xóa', '', 'success'); }
...
<template>
    <div class="app-content-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-6"><h3 class="mb-0 text-brand">Quản lý Sản phẩm</h3></div>
                <div class="col-sm-6">
                    ...
                    <div class="row align-items-center g-3">
                        <div class="col-md-4 col-12">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control border-start-0 ps-0" placeholder="Tìm kiếm sản phẩm..." v-model="searchQuery">
                            </div>
                        </div>
                        ...
                        <div class="col-md-2 col-12 text-end">
                            <button class="btn btn-primary w-100 text-nowrap shadow-sm" @click="openCreateModal"><i class="bi bi-plus-lg me-1"></i> Thêm mới</button>
                        </div>
                    </div>
                </div>
                                ...
                                <tr v-else v-for="p in paginatedProducts" :key="p.id">
                                    ...
                                    <td>
                                        <div class="ratio ratio-1x1 rounded overflow-hidden shadow-sm" style="width: 50px;">
                                            <img :src="p.thumbnail_url" class="object-fit-cover" onerror="this.src='https://placehold.co/50?text=Err'">
                                        </div>
                                    </td>
                                    <td><span class="fw-bold text-dark d-block text-truncate" style="max-width: 250px;" :title="p.name">{{ p.name }}</span></td>
                                    <td><span class="badge bg-light text-dark border">{{ p.category?.name || 'N/A' }}</span></td>
                                    <td class="text-danger fw-bold">{{ getPriceRange(p.variants) }}</td>
                                    ...
                                    <td class="text-center pe-3">
                                        <div class="d-flex justify-content-center gap-1">
                                            <button class="btn btn-sm btn-light text-secondary border" @click="openViewModal(p)" title="Xem"><i class="bi bi-eye"></i></button>
                                            <button class="btn btn-sm btn-light text-primary border" @click="openVariantConfigModal(p)" title="Sửa"><i class="bi bi-pencil"></i></button>
                                            <button class="btn btn-sm btn-light text-danger border" @click="handleDelete(p)" title="Xóa"><i class="bi bi-trash"></i></button>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
                    ...
                    <div class="card-footer bg-white border-top-0 py-3" v-if="totalPages > 1">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <ul class="pagination pagination-sm m-0">
                                <li class="page-item" :class="{ disabled: currentPage === 1 }"><button class="page-link border-0" @click="goToPage(currentPage - 1)"><i class="bi bi-chevron-left"></i></button></li>
                                <li class="page-item disabled"><span class="page-link border-0 text-dark">{{ currentPage }} / {{ totalPages }}</span></li>
                                <li class="page-item" :class="{ disabled: currentPage === totalPages }"><button class="page-link border-0" @click="goToPage(currentPage + 1)"><i class="bi bi-chevron-right"></i></button></li>
                            </ul>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header bg-light">
                    <h5 class="modal-title fw-bold text-brand">{{ isEditMode ? 'Cấu hình Sản phẩm & Biến thể' : 'Thêm Sản phẩm mới' }}</h5>
                ...
                <div class="modal-body p-4">
                    <div class="row g-4">
                        ...
                        <div class="col-lg-4 border-end">
                            <h6 class="fw-bold text-secondary mb-3 text-uppercase small">Thông tin chung</h6>
                            <div class="mb-3">
                                <label class="form-label required fw-bold">Tên sản phẩm</label>
                                <input type="text" class="form-control" v-model="formData.name" :class="{ 'is-invalid': errors.name }" placeholder="Nhập tên sản phẩm...">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" rows="4" v-model="formData.description" placeholder="Mô tả chi tiết..."></textarea>
                            </div>
                            ...
                            <div class="p-3 bg-light rounded border border-dashed text-center">
                                ...
                                <input type="file" class="form-control form-control-sm mt-2" accept="image/*" @change="handleThumbnailChange">
                                <div class="form-text small mt-1">Tối thiểu 500x500px, Max 5MB.</div>
                            </div>
                        </div>

                        <!-- Right Column: Variants & Gallery OR Live Preview -->
                        <div class="col-lg-8">
                            <!-- [UPDATED] Live Preview for New Product (Fixed Layout) -->
                            <div v-if="!isEditMode" class="h-100">
                                <div class="card shadow-sm border-0 h-100">
                                    <div class="card-header bg-white border-bottom-0 pt-3 ps-3">
                                        <h6 class="fw-bold text-muted text-uppercase small mb-0"><i class="bi bi-eye me-1"></i> Xem trước (Live Preview)</h6>
                                    </div>
                                    <div class="card-body d-flex flex-column justify-content-center align-items-center bg-light m-3 rounded border border-dashed">
                                         <!-- Actual Card Preview -->
                                         <div class="card shadow-sm border-0" style="width: 18rem; overflow: hidden;">
                                            <div class="ratio ratio-4x3 bg-white">
                                                <img :src="thumbnailPreview || 'https://placehold.co/300x200?text=Sản+Phẩm'" class="card-img-top object-fit-contain p-3">
                                            </div>
                                            <div class="card-body text-start">
                                                ...
                                                <!-- [FIXED] Force block display + text-truncate properly -->
                                                <h5 class="card-title fw-bold text-dark mb-1 d-block text-truncate w-100" :title="formData.name">
                                                    {{ formData.name || 'Tên sản phẩm...' }}
                                                </h5>
                                                <p class="card-text small text-muted mb-3 d-block text-truncate w-100">
                                                    {{ formData.description || 'Mô tả ngắn của sản phẩm sẽ hiển thị tại đây...' }}
                                                </p>
                                                <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div v-else>
                                ...
                                <div class="mb-4">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <h6 class="fw-bold text-secondary text-uppercase small mb-0">Thư viện ảnh</h6>
                                        <div v-if="formData.existing_images.length > 0">
                                            <input class="form-check-input cursor-pointer me-2" type="checkbox" id="selectAll" v-model="isAllImagesSelected">
                                            <label class="form-check-label small me-3 cursor-pointer" for="selectAll">Chọn tất cả</label>
                                            <button v-if="selectedImageIds.length > 0" class="btn btn-xs btn-danger" @click="handleBulkDeleteImages" :disabled="isBulkDeleting">
                                                <i class="bi bi-trash me-1"></i> Xóa ({{ selectedImageIds.length }})
                                            </button>
                                        </div>
                                    </div>
                                    <div class="p-3 border rounded bg-white">
                                        <div class="d-flex flex-wrap gap-2 mb-3">
                                            <div v-for="(img, idx) in formData.existing_images" :key="img.id" class="position-relative">
                                                <img :src="img.url" class="img-thumbnail" :class="{ 'border-primary shadow-sm': selectedImageIds.includes(img.id) }" style="width: 80px; height: 80px; object-fit: cover; cursor: pointer;" @click="selectedImageIds.includes(img.id) ? selectedImageIds = selectedImageIds.filter(id => id !== img.id) : selectedImageIds.push(img.id)">
                                                <div class="position-absolute top-0 start-0 m-1"><input type="checkbox" class="form-check-input" :value="img.id" v-model="selectedImageIds" @click.stop></div>
                                            </div>
                                            <div v-for="(url, idx) in galleryPreviews" :key="'new-'+idx" class="position-relative">
                                                <img :src="url" class="img-thumbnail border-success" style="width: 80px; height: 80px; object-fit: cover;">
                                                <button class="btn btn-sm btn-secondary rounded-circle position-absolute top-0 start-100 translate-middle p-0" style="width: 20px; height: 20px; line-height: 1;" @click="removeNewGalleryImage(idx)">&times;</button>
                                            </div>
                                            <label class="border border-dashed rounded d-flex align-items-center justify-content-center cursor-pointer bg-light text-muted hover-bg-gray" style="width: 80px; height: 80px;">
                                                <div class="text-center"><i class="bi bi-plus-lg fs-4"></i><div style="font-size: 10px;">Thêm ảnh</div></div>
                                ...
                                <div>
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <h6 class="fw-bold text-secondary text-uppercase small mb-0">Cấu hình biến thể</h6>
                                        <div class="d-flex gap-2">
                                            ...
                                            <button class="btn btn-sm btn-primary" @click="addSelectedAttributeToForm" :disabled="!selectedAttributeId">Thêm</button>
                                            <button class="btn btn-sm btn-outline-success" @click="openCreateAttrModal">Tạo mới</button>
                                        </div>
                                    </div>
                                    <div class="table-responsive border rounded">
                                        <table class="table table-sm table-bordered text-center align-middle mb-0 bg-white">
                                            <thead class="bg-light">
                                                <tr>
                                                    <th v-for="d in formData.attribute_definitions" :key="d.id" style="min-width: 100px;">
                                                        {{ d.name }} <i class="bi bi-x text-danger cursor-pointer ms-1" title="Xóa cột" @click="removeAttributeDefinition(d)"></i>
                                                    </th>
                                            ...
                                            <tbody>
                                                <tr v-for="(v, i) in formData.variants" :key="i">
                                                    <td v-for="d in formData.attribute_definitions" :key="d.id">
                                                        <input type="text" class="form-control form-control-sm text-center border-0" v-model="v.attributes[d.name]" placeholder="...">
                                                    </td>
                                                    <td><input type="number" class="form-control form-control-sm text-end" v-model="v.original_price"></td>
                                                    <td><input type="number" class="form-control form-control-sm text-end fw-bold text-primary" v-model="v.price"></td>
                                                    <td><input type="number" class="form-control form-control-sm text-center" v-model="v.stock"></td>
                                                    <td><button class="btn btn-sm text-danger" @click="removeVariantRow(i)">&times;</button></td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <button class="btn btn-sm btn-light border w-100 mt-2 text-primary fw-bold" @click="addVariantRow"><i class="bi bi-plus-lg"></i> Thêm biến thể</button>
                                    <div class="text-danger small mt-2" v-if="errors.variants">{{ errors.variants }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer bg-light border-top-0">
                    <button class="btn btn-outline-secondary px-4" data-bs-dismiss="modal">Đóng</button>
                    ...
                    <button v-if="isEditMode" class="btn btn-primary px-4" @click="handleSave" :disabled="isSaving">
                        <span v-if="isSaving" class="spinner-border spinner-border-sm me-1"></span> Lưu thay đổi
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="attrModal" ref="attrModalRef" tabindex="-1">
        <div class="modal-dialog modal-sm modal-dialog-centered">
            <div class="modal-content shadow">
                <div class="modal-header bg-success text-white py-2"><h6 class="modal-title mb-0">Tạo thuộc tính mới</h6><button class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
                <div class="modal-body"><input type="text" class="form-control" v-model="newQuickAttrName" placeholder="VD: Màu sắc, Size..."></div>
                <div class="modal-footer p-2"><button class="btn btn-sm btn-success w-100" @click="handleCreateQuickAttribute" :disabled="isCreatingAttr">{{ isCreatingAttr ? '...' : 'Tạo ngay' }}</button></div>
            </div>
        </div>
    </div>
                ...
                <div class="modal-body p-0">
                    <div class="row g-0">
                        <div class="col-md-5 bg-light d-flex align-items-center justify-content-center p-4">
                            <img :src="viewingProduct.thumbnail_url" class="img-fluid rounded shadow-sm" style="max-height: 300px;">
                        </div>
                        <div class="col-md-7 p-4">
                            <button type="button" class="btn-close float-end" data-bs-dismiss="modal"></button>
                            <span class="badge bg-primary mb-2">{{ viewingProduct.categoryName }}</span>
                            <h4 class="fw-bold mb-3">{{ viewingProduct.name }}</h4>
                            <div class="mb-3">
                                <h2 class="text-danger fw-bold mb-0">{{ viewingProduct.priceRange }}</h2>
                                <small class="text-muted">Kho: {{ viewingProduct.totalStock }}</small>
                            </div>
                            <p class="text-muted small mb-4" style="white-space: pre-line;">{{ viewingProduct.description }}</p>
                            <hr>
                            <h6 class="fw-bold small text-uppercase text-secondary">Chi tiết Biến thể ({{ viewingProduct.variants.length }})</h6>
                            <div style="max-height: 200px; overflow-y: auto;">
                                <div v-for="v in viewingProduct.variants" :key="v.id" class="mb-2 p-2 border rounded bg-light">
                                    <div class="d-flex justify-content-between align-items-center mb-1">
                                        <div>
                                            <span v-for="(val, key) in v.attributes" :key="key" class="badge bg-white text-dark border me-1">
                                                {{ key }}: {{ val }}
                                            </span>
                                        </div>
                                        <span class="fw-bold text-danger">{{ formatCurrency(v.price) }}</span>
                                    </div>
                                    <div class="small text-muted d-flex justify-content-between">
                                        <span>Kho: <b>{{ v.stock }}</b></span>
