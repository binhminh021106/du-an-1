    ...
    <div class="app-content">
        <div class="container-fluid">
            ...
            <ul class="nav nav-pills mb-4 bg-white p-2 rounded shadow-sm d-inline-flex">
                <li class="nav-item"><a class="nav-link cursor-pointer fw-bold px-4" :class="{ active: activeDashboardTab === 'overview' }" @click="activeDashboardTab = 'overview'"><i class="bi bi-grid-1x2-fill me-2"></i>Tổng quan</a></li>
                <li class="nav-item"><a class="nav-link cursor-pointer fw-bold px-4" :class="{ active: activeDashboardTab === 'revenue' }" @click="activeDashboardTab = 'revenue'"><i class="bi bi-graph-up-arrow me-2"></i>Doanh thu</a></li>
            </ul>
            ...
            <div v-if="activeDashboardTab === 'overview'" class="row g-4 fade-in">
                        ...
                        <div class="col-md-6">
                            <div class="card shadow-sm border-0 h-100">
                                ...
                                <div class="list-group list-group-flush flex-grow-1">
                                    ...
                                    <div v-for="review in latestReviews" :key="review.id" class="list-group-item px-3 py-2 border-bottom-0">
                                        <div class="d-flex justify-content-between mb-1"><small class="fw-bold text-truncate" style="max-width: 120px;">{{ review.product?.name }}</small><span class="text-warning small" style="letter-spacing: -1px;">{{ renderStars(review.rating) }}</span></div>
                                        <p class="mb-0 small text-muted text-truncate fst-italic">"{{ review.content }}"</p>
                                    </div>
                                </div>
                                <div class="card-footer bg-white border-top-0 text-center py-3">
                                    <router-link to="/admin/reviews" class="btn btn-sm btn-light text-brand fw-bold border px-4 shadow-sm hover-brand">Xem thêm</router-link>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="card shadow-sm border-0 h-100">
                                ...
                                <div class="list-group list-group-flush flex-grow-1">
                                    ...
                                    <div v-for="cmt in latestComments" :key="cmt.id" class="list-group-item px-3 py-2 border-bottom-0">
                                        <div class="d-flex align-items-center mb-1"><img :src="getAvatar(cmt.user?.avatar, cmt.user?.username)" class="rounded-circle me-2" width="20"><span class="fw-bold small text-dark">{{ cmt.user?.username || 'Ẩn danh' }}</span></div>
                                        <p class="mb-0 small text-muted text-truncate">"{{ cmt.content }}"</p>
                                    </div>
                                </div>
                                <div class="card-footer bg-white border-top-0 text-center py-3">
                                    <router-link to="/admin/comments" class="btn btn-sm btn-light text-brand fw-bold border px-4 shadow-sm hover-brand">Xem thêm</router-link>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-lg-4">
                    <!-- CHART -->
                    <div class="card shadow-sm border-0 mb-4">
                        <div class="card-header bg-white border-bottom py-3"><h5 class="card-title fw-bold text-brand mb-0">Tình Trạng Đơn Hàng</h5></div>
                        <div class="card-body">
                            <div style="height: 220px;" class="mb-3 position-relative"><canvas ref="chartCanvas"></canvas></div>
                            <!-- Legend -->
                            <div class="status-list small">
                                <div class="d-flex justify-content-between align-items-center mb-2 pb-1 border-bottom">
                                    <span class="fw-bold text-secondary">Tổng</span><span class="fw-bold text-dark">{{ stats.totalOrders }} đơn</span>
                                </div>
                                <div v-for="(config, key) in STATUS_CONFIG" :key="key"
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="dot rounded-circle me-2" :style="{ backgroundColor: config.color }"></div>
                                        <span class="text-dark">{{ config.label }}</span>
                                    </div>
                                    <span class="fw-bold badge bg-light text-dark border">{{ orderStatusCounts[key] || 0 }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card shadow-sm border-0">
                        ...
                        <div class="list-group list-group-flush">
                            <div v-for="user in newMembers" :key="user.id" class="list-group-item px-3 py-3 d-flex align-items-center">
                                <img :src="getAvatar(user.avatar_url, user.fullname)" class="rounded-circle me-3 border shadow-sm" width="36" height="36" style="object-fit: cover;">
                                <div class="flex-grow-1 min-width-0">
                                    <div class="fw-bold text-dark text-truncate small">{{ user.fullname }}</div>
                                    <div class="small text-muted text-truncate" style="font-size: 0.75rem;">{{ user.email }}</div>
                                </div>
                                <div class="small text-muted">{{ formatDate(user.created_at) }}</div>
                            </div>
                        </div>
                        <div class="card-footer bg-white border-top-0 text-center py-3">
                            <router-link to="/admin/userAccount" class="btn btn-sm btn-light text-brand fw-bold border px-4 shadow-sm hover-brand">Xem thêm</router-link>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-if="activeDashboardTab === 'revenue'" class="row g-4 fade-in">
                <div class="col-12">
                    <div class="row g-3">
                        <div class="col-md-6"><div class="card bg-light border-0 p-3 h-100"><h6 class="text-muted text-uppercase small fw-bold">Doanh thu năm nay</h6><h3 class="text-brand fw-bold mb-0">{{ formatCurrency(stats.totalRevenue) }}</h3></div></div>
                        <div class="col-md-3"><div class="card bg-light border-0 p-3 h-100"><h6 class="text-muted text-uppercase small fw-bold">TB Đơn hàng</h6><h4 class="text-dark fw-bold mb-0">{{ formatCurrency(stats.avgOrderValue) }}</h4></div></div>
                        <div class="col-md-3"><div class="card bg-light border-0 p-3 h-100"><h6 class="text-muted text-uppercase small fw-bold">Tháng cao điểm</h6><h4 class="text-success fw-bold mb-0">{{ stats.bestMonth }}</h4></div></div>
                    </div>
                </div>
